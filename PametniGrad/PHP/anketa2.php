<?php	
	if (isset($_POST['btnZdrastvo'])){
		$insert = "INSERT INTO anketa (Kor_ime, anketa, pitanje1, pitanje2, pitanje3, pitanje4, pitanje5, pitanje6, pitanje7, pitanje8, pitanje9, pitanje10) 
				               VALUES ('{$_SESSION['korisnik']}','zdrastvo','{$_POST['t1']}','{$_POST['t2']}','{$_POST['t3']}','{$_POST['t4']}','{$_POST['t5']}','{$_POST['t6']}','{$_POST['t7']}','{$_POST['t8']}','{$_POST['t9']}','{$_POST['t10']}')";
		...
		if(!empty($_POST['t1']) && !empty($_POST['t2'])&& !empty($_POST['t3'])
										...
										&& !empty($_POST['t10'])  ){
		$update = "UPDATE korisnici SET zdrastvo = 1 WHERE Kor_ime = '{$_SESSION['korisnik']}' ";
